...
function isReceipt(value: unknown): value is Receipt {
  ...
  return ["id", "registeredAt", "company", "name", "email"].every(key => typeof data[key] === "string");
}
export function ReceiptForm() {
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>) {
    ...
    try {const r=await fetch("/api/actualizaciones",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({requestId,version:notice.version,consent:confirmed,company:form.get("company"),nit:form.get("nit"),name:form.get("name"),email:form.get("email")})});const body:unknown=await r.json();if(!r.ok)throw new Error(body&&typeof body==="object"&&"error" in body&&typeof body.error==="string"?body.error:"No pudimos registrar tus datos. Intenta nuevamente.");if(!isReceipt(body))throw new Error("La respuesta no tiene el formato esperado. Intenta nuevamente.");setReceipt(body);}catch(e){setError(e instanceof Error?e.message:"No se pudo guardar. Comprueba tu conexión e intenta de nuevo.");}finally{setBusy(false);}
  }
