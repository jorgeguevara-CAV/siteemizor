...
export function ReceiptForm() {
  ...
  return <form className="form-card" onSubmit={submit}><h3>Recibe nuestras novedades</h3><p className="form-intro">Indícanos cómo contactarte para mantenerte al día.</p><div className="fields"><label htmlFor="name">Tu nombre completo<Input id="name" name="name" required minLength={3} maxLength={160} autoComplete="name" placeholder="Nombre y apellido"/></label><label htmlFor="email">Correo electrónico<Input id="email" name="email" type="email" required maxLength={180} autoComplete="email" placeholder="nombre@empresa.com"/></label><label htmlFor="company">Empresa o razón social<Input id="company" name="company" required minLength={2} maxLength={180} autoComplete="organization" placeholder="Nombre de tu empresa"/></label><label htmlFor="nit">NIT de la empresa (opcional)<Input id="nit" name="nit" pattern="[0-9]{5,20}" inputMode="numeric" maxLength={20} title="Ingresa entre 5 y 20 dígitos, sin espacios ni guiones" placeholder="Solo números"/></label></div><div className="consent"><Checkbox id="confirmed" checked={confirmed} onCheckedChange={v=>setConfirmed(v===true)}/><label htmlFor="confirmed">Autorizo a EMIZOR a utilizar estos datos para enviarme novedades sobre la adecuación a la Ley N.º 1733.</label></div><p className="form-note">Podrás solicitar dejar de recibir estas novedades a través de tu canal habitual de atención con EMIZOR.</p>{error&&<p className="form-error" role="alert">{error}</p>}<Button className="submit-button" type="submit" disabled={busy||!requestId}>{busy?"Registrando tus datos…":"Quiero mantenerme informado"} {!busy&&<ArrowUpRight size={18}/>}</Button></form>;
}
